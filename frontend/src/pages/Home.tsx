  ...
  return (
    <main className="min-h-screen bg-gray-100 overflow-hidden">
      <header className="h-14 sm:h-16 bg-gray-300 flex items-center justify-between px-3 sm:px-4 shadow">
        ...
        {/* ← removido o círculo, ficou só o botão de logout */}
        <button
          onClick={handleLogout}
          className="flex items-center gap-2 text-sm text-gray-700 hover:text-red-500 transition"
        >
          <LogOut size={20} />
          Sair
        </button>
      ...
      >
        ...
        <nav className="flex flex-col gap-3 p-4">
          <button
            onClick={() => { setSidebarAberta(false); navigate("/atividades/ativas"); }}
          ...
          <button
            onClick={() => { setSidebarAberta(false); navigate("/atividades/concluidas"); }}
            className="text-left px-4 py-3 rounded-xl bg-gray-100 hover:bg-gray-300 hover:shadow-md hover:scale-[1.02] transition-all duration-200 font-medium"
          >
      ...
      <section className="p-4 sm:p-6">
        ...
        >
          ...
          <span className="text-lg sm:text-2xl">CRIAR ATIVIDADE</span>
        </button>
      </section>
    </main>
  );
